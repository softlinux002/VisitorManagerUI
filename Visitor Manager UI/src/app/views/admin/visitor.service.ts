...
//   }
  ...
  getVisitors(name:string, date:string, vehNo:string, adminId:string):Observable<any>    
  {    
    const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) };    
    return this.http.get<any>(this.Url + '/Account/GetVisitorByFilter?name='+name+'&date='+date+'&vehNo='+vehNo+'&loggedAdminId='+adminId);
  }
...
//   CreateStaff(OutletVM:VisitorVM):Observable<any>    
//   {    
//    const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json' }) };
//    if(OutletVM.Id>0) {
